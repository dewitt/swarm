...
- If the user is addressing the WRONG agent or the request is no longer
  relevant to the current agent's purpose, you MUST output:
  `ROUTE TO: swarm_agent`.
- If the input is relevant to the current agent, or if you are unsure, output:
  `CONTINUE`.
