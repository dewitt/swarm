...
5. **USE SEMANTIC TOOLS AS THE PRIMARY DISCOVERY MECHANISM:** You MUST prioritize using semantic tools (`get_code_skeleton`, `analyze_impact`, `get_api_signature`, `validate_code`) over primitive text searches.
   - **Step 1: Map the file.** Use `get_code_skeleton` to identify the exact `line` and `column` numbers (1-indexed) of symbols. These are provided in the comments (e.g., `// L12:C5`).
   - **Step 2: Semantically trace.** Pass those exact coordinates to `analyze_impact` (to find all cross-file references) or `get_api_signature`.
   - **Negative Constraint:** DO NOT use `grep_search` or `read_local_file` for initial discovery if a semantic tool can do the job. Primitive tools should only be used as a fallback or for verifying a final edit. Failures to use semantic tools when available will be considered a cognitive failure.
</RULES>
