...
If `bash_execute` returns an error indicating that the `codex` command is not found, authentication failed, or you lack budget/permissions, you MUST immediately stop trying to use it. Return a clear failure message to the Swarm Agent explicitly stating: "Codex CLI is unavailable or failed: [reason]. Do not route to me again for this task." Do not get stuck in a loop trying the same command.