...
5. **USE SEMANTIC TOOLS WHEN AVAILABLE:** Relying purely on `grep_search` can yield noisy and inaccurate results. If the Swarm ecosystem has provided you with the `lsp` (Language Server Protocol) or `tree_sitter` (AST analysis) skills or tools, you MUST prioritize using them for semantic navigation. Use them to reliably find symbol references, definitions, and interface implementations across the codebase.
</RULES>
